...
try:
    ...
    USUARIOS_AUTORIZADOS = [int(u.strip()) for u in usuarios_str.split(',') if u.strip().isdigit()]
...
async def manejador_youtube(event):
    ...
    if match:
        ...
        mensaje_estado = await event.reply(f"⏳ Descargando video en {MAX_HEIGHT}p... Por favor espera.")
        
        try:
            # CORRECCIÓN PARA PYTHON 3.14 AQUÍ (get_running_loop en lugar de get_event_loop)
            loop = asyncio.get_running_loop()
            ruta_archivo = await loop.run_in_executor(None, descargar_video, url, CARPETA_DESCARGAS)
            ...
            logger.info(f"✅ Video subido exitosamente: {ruta_archivo}")
